...
export class InteractiveMode implements InteractiveModeContext {
	...
	async init(options: InteractiveModeInitOptions = {}): Promise<void> {
		...
		} else if (this.settings.get("tui.fullscreen")) {
			// Prepaint already started the TUI, so the start listener above will
			// never fire for this launch: enter fullscreen now instead.
			this.setFullscreenTui(true);
		}
		pushTerminalTitle();
	...
	setFullscreenTui(enabled: boolean): void {
		if (enabled) {
			...
			this.composer.setPreferences({ showHardwareCursor: false });
		...
		this.composer.setPreferences({ showHardwareCursor: this.settings.get("showHardwareCursor") });
