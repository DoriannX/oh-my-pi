...
export class TUI extends Container {
	...
	#renderResizeAltFrame(width: number, height: number): void {
		...
		this.#emitAltFrame(this.#prepareLinesArray(viewport, width, this.#altPreparedRows, height), width, height, null);
	}
		...
		const wantMouse: MouseTrackingState =
			topOverlay === undefined
				? this.#inlineMouseProvider?.() === true
					? "inline"
					: "off"
				: topOverlay === fullscreenOverlay && fullscreenOverlay.options?.mouseTracking !== false
					? "full"
					: "off";
	...
	#renderAltFrame(width: number, height: number): void {
		...
		this.#emitAltFrame(prepared, width, height, target);
	}
	 ...
	 */
	#emitAltFrame(prepared: PreparedLines, width: number, height: number, target: HardwareCursorState | null): void {
